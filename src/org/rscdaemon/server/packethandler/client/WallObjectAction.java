...
public class WallObjectAction implements PacketHandler {
	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		world.getDelayedEventHandler().add(
				new WalkToPointEvent(player, object.getLocation(), 1, false) {
					...
					public void arrived() {
						...
						if (telePoint != null) {
							...
							if (command.equals("pick lock")) {
								...
								if (!Formulae.catchThief(owner.getCurStat(17),
										55)) {
									if (owner.getLocation().getX() == 539
											&& owner.getLocation().getY() == 599) {
										owner.teleport(539, 598, false);
									} else if (owner.getLocation().getX() == 539
											&& owner.getLocation().getY() == 598) {
										owner.teleport(539, 599, false);
									}
									world.registerGameObject(new GameObject(
											object.getLocation(), 1, object
													.getDirection(), object
													.getType()));
							...
							case 2:
								if (object.getX() != 374
									...
									Npc affectedNpc = world.getNpc(206, 374,
											374, 3331, 3331);
									Npc affectedNpc2 = world.getNpc(206, 374,
											374, 3333, 3333);
									if (affectedNpc == null) {
									...
									if (affectedNpc != null
										...
										owner.getActionSender()
												.sendMessage(
														"Suddenly the suit of armour comes to life!");
									...
									if (affectedNpc == null
										...
										world.delayedSpawnObject(
												object.getLoc(), 1000);
									} else if (owner.getLocation().getX() == 373
											&& owner.getLocation().getY() == 3332) {
							...
							case 112: // Fishing Guild Door
								...
								if (owner.getY() > 523) {
									if (owner.getCurStat(10) < 68) {
										owner.setBusy(true);
										Npc masterFisher = world.getNpc(368,
												582, 588, 524, 527);
							...
							case 55: // Mining Guild Door
								...
								if (owner.getY() <= 3380) {
									if (owner.getCurStat(14) < 66) {
										owner.setBusy(true);
										Npc dwarf = world.getNpc(191, 265, 270,
												3379, 3380);
										if (dwarf != null) {
											owner.informOfNpcMessage(new ChatMessage(
													dwarf,
							...
							case 68: // Crafting Guild Door
								...
								if (owner.getY() <= 600) {
									...
									} else if (!owner.getInventory().wielding(
											191)) {
										Npc master = world.getNpc(231, 341,
												349, 599, 612);
										if (master != null) {
							...
							case 43: // Cooking Guild Door
								...
								if (owner.getY() >= 488) {
									if (owner.getCurStat(7) < 32) {
										owner.setBusy(true);
										Npc chef = world.getNpc(133, 176, 181,
												480, 487);
										if (chef != null) {
											owner.informOfNpcMessage(new ChatMessage(
													chef,
							...
							case 94:
								...
								break;
							case 20:
								if (owner.getLocation().getX() == 199
										&& owner.getLocation().getY() == 652) {
									if (owner.getInventory().countId(525) < 1) {
										// owner.setBusy(true);
										owner.getActionSender()
												.sendMessage(
														"You can not enter Hixk's House without a key");
										break;
									} else {
										doDoor();
										owner.teleport(199, 653, false);
										break;
									}
								} else {
									doDoor();
									owner.teleport(199, 652, false);
									break;
								}
							default:
								owner.getActionSender().sendMessage(
