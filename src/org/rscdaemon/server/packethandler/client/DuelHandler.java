	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		case 222: // Sending duel request
			...
			if (!player.getLocation().inWaitingRoom()
					&& !affectedPlayer.getLocation().inWaitingRoom()
					&& !player.getLocation().inDuelArena()
					&& !affectedPlayer.getLocation().inDuelArena()) {
				...
				player.getActionSender().sendMessage(
						affectedPlayer.isDueling() ? affectedPlayer
								.getUsername() + " is already in a duel"
				...
				player.getActionSender().sendMessage("You can not duel here!");
			}
			break;
