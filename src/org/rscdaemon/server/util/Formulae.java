...
import org.rscdaemon.server.packethandler.client.SpellHandler;
...
/**
 * Formulas created as close to actual rsc as possible.
 * 
 * @author Devin
 */
...
public class Formulae {
	...
	public static final int[] experienceArray = { 83, 174, 276, 388, 512, 650,
			...
			14391160, 15747889, 17104618, 18461347, 19818076, 21174805,
			22531534, 23888263, 25244992, 26601721, 27958450, 29315179,
			30671908, 32028637, 33385366, 34742095, 36098824, 37455553,
			38812282, 40169011, 41525740, 42882469, 44239198, 45595927,
			46952656, 48309385, 49666114, 51022843, 52379572, 53736301,
			55093030, 56449759, 57806488, 59163217, 60519946, 61876675,
			63233404, 64590133, 65946862, 67303591, 68660320, 70017049,
			71373778, 72730507, 74087236, 75443965, 76800694, 78157423,
			79514152, 80870881, 82227610, 83584339 };
	public static final int[] eArray = { 0, 0, 83, 174, 276, 388, 512, 650,
			801, 969, 1154, 1358, 1584, 1833, 2107, 2411, 2746, 3115, 3523,
			3973, 4470, 5018, 5624, 6291, 7028, 7842, 8740, 9730, 10824, 12031,
			13363, 14833, 16456, 18247, 20224, 22406, 24815, 27473, 30408,
			33648, 37224, 41171, 45529, 50339, 55649, 61512, 67983, 75127,
	...
	public static String delayToString(long delay) {
		...
		if (days > 0)
			dayStr = days + " days, ";
		return dayStr + df2.format(hours % 24) + ":" + df2.format(minutes % 60)
				+ ":" + df2.format(seconds % 60);
	}
	...
	public static boolean catchThief(int lvl, int reqLevel) {
		double dif = lvl - reqLevel; // Get difference
	...
	/*
	 * public static int calculateMaxRangeHit(double arrowMod, double bowMod,
	 * int rng) { double rawResult = (bowMod * arrowMod * rng) * 0.1;
	 * System.out.println("bow="+bowMod+"\tarrow="+arrowMod); return
	 * (int)Math.ceil(rawResult); }
	 * 
	 * public static int calculateRangeHit(Mob att, Mob def, double arrowMod,
	 * double bowMod) { final int[] FIGHT_MODE_DEF_BONUSES = new int[] {1, 0, 0,
	 * 3}; int max = calculateMaxRangeHit(arrowMod, bowMod, att.getCurStat(4));
	 * int ourAttack = (int)att.getCurStat(4); double wepAim =
	 * att.getRangePoints()*2.5; int enemyDef =
	 * ((int)(def.getCurStat(1)*def.getModifiers
	 * ()[1]))+FIGHT_MODE_DEF_BONUSES[def.getCombatStyle()]; double defPoints =
	 * def.getArmourPoints()*0.47; double probabilityOfHit = ((ourAttack +
	 * wepAim) - (enemyDef + defPoints)) / 100 + 0.6; double rand =
	 * Math.random(); double maxrand = Math.random(); int hit = 0;
	 * if(probabilityOfHit < ourAttack*0.001) probabilityOfHit =
	 * ourAttack*0.001; if(probabilityOfHit > 1) maxrand += Math.random() *
	 * ((probabilityOfHit-1)/10); if(maxrand > 1) maxrand = 1; //
	 * if(probabilityOfHit > rand) // hit = (int)(Math.random() * max);
	 * if(probabilityOfHit > rand) hit = (int)(maxrand * max);
	 * System.out.println("["+(att instanceof Player ? "Plyr" : "NPC"
	 * )+"]\thitRate="
	 * +(probabilityOfHit*100)+"%\tatt="+ourAttack+"\twepAim="+wepAim
	 * +"\t def="+enemyDef+"\tdefPoints="+defPoints); //
	 * System.out.println(att+" hit a "+hit+" on "+def);
	 * System.out.println(max); return hit; }
	 */
	public static int calcGodSpells(Mob attacker, Mob defender) {
		if (attacker instanceof Player) {
			...
			int hitChance = DataConversions.random(0, 120 + (newAtt - newDef));
	...
	public static int calcFightHit(Mob att, Mob def) {
		final int[] FIGHT_MODE_ATT_BONUSES = new int[] { 1, 0, 3, 0 };
		final int[] FIGHT_MODE_DEF_BONUSES = new int[] { 1, 0, 0, 3 };
		final double[] mods = att.getModifiers();
		int max = calculateMaxCombatHit(mods[2], att.getCurStat(2),
				att.getCombatStyle(), att.getWeaponPowerPoints());
		int ourAttack = ((int) (att.getCurStat(0) * mods[0]))
				+ FIGHT_MODE_ATT_BONUSES[att.getCombatStyle()];
		double wepAim = att.getWeaponAimPoints() * 0.8;
		int enemyDef = ((int) (def.getCurStat(1) * def.getModifiers()[1]))
				+ FIGHT_MODE_DEF_BONUSES[def.getCombatStyle()];
		...
		int hit = 0;
		// System.out.println("["+(att instanceof Player ? "Plyr" : "NPC"
		// )+"]\thitRate="+(probabilityOfHit*100)+"%\tatt="+ourAttack+"\twepAim="+wepAim+"\t def="+enemyDef+"\tdefPoints="+defPoints);
	...
	/*
	 * public static int calcFightHit(Mob attacker, Mob defender) { int newAtt =
	 * (int)((addPrayers(attacker.isPrayerActivated(2),
	...
	public static int calcSpellHit(Player p, Mob target, int spellDmg) {
		double probabilityOfHit = ((p.getCurStat(6) + 0.9 * p.getMagicPoints()) - (target
				.getCurStat(1) * 0.3 + 0.9 * target.getMagicPoints())) / 100;
		double rand = Math.random();
		int hit = 0;
		...
		// System.out.println("probability="+probabilityOfHit+", hit="+hit);
		return hit;
	}
	...
	public static int experienceToLevel(int exp) {
		for (int level = 0; level < 119; level++) {
