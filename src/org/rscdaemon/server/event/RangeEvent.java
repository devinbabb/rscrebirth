...
public class RangeEvent extends DelayedEvent {
	...
	public void run() {
		...
		// int damage = Formulae.calculateRangeHit(owner, affectedMob,
		// Formulae.arrowPower(arrowID), bowStats[1]);

		/*
		 * try { damage = Formulae.calculateRangeHit(owner, affectedMob,
		 * getArrowMod(arrowID), bowStats[1]); } catch(NullPointerException npe)
		 * { // setRunning(false); return; System.out.println(npe); return; }
		 */
		...
		if (affectedMob instanceof Npc && owner.withinRange(affectedMob, 6)
				&& !affectedMob.isBusy() && !owner.isBusy()) {
			final Npc affectedNpc = (Npc) affectedMob;
			// affectedMob.moveNpc(new Path(affectedNpc.getX(),
			// affectedNpc.getY(), owner.getLocation().getX(),
			// owner.getLocation().getY()));
			final Player victim = owner;
			if (victim != null) {
				world.getDelayedEventHandler().add(
						new NpcWalkEvent(owner, affectedNpc, 0) {
							public void arrived() {
								...
								for (Player p : affectedNpc.getViewArea()
										.getPlayersInView()) {
								...
								FightEvent fighting = new FightEvent(victim,
										affectedNpc, true);
