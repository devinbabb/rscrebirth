...
/**
 * Controls sending update information to client.
 * 
 * @author Devin
 */
...
public final class ClientUpdater {
	...
	private void updateTimeouts(Player p) {
		if (!p.getLocation().inWaitingRoom() || !p.getLocation().inDuelArena()) {
			if (p.destroyed()) {
				return;
			}
			...
			if (curTime - p.getLastPing() >= 30000) {
				...
				p.getActionSender()
						.sendMessage(
								"@cya@You have not moved for 5 mins, please move to a new area to avoid logout.");
				p.warnToMove();
			}
		}
	}
